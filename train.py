...
from utils.utils import AverageMeter, initialize_logger, save_checkpoint, record_loss
from utils.loss import LossTrainCSS, Loss_valid
...
parser.add_argument("--init_lr", type=float, default=1.5e-4, help="initial learning rate")
parser.add_argument("--decay_power", type=float, default=1.5, help="decay power")
parser.add_argument("--trade_off", type=float, default=0.1, help="trade_off")
parser.add_argument("--max_iter", type=float, default=300000, help="max_iter")
parser.add_argument("--outf", type=str, default="NTIRE2020/ckpt", help='path log files')
...
def train(train_loader, model, criterion, optimizer, epoch, iteration, init_lr, decay_power, trade_off):
    ...
    for k, train_data_loader in enumerate(train_loader):
        for i, (images, labels) in enumerate(train_data_loader):
            ...
            output = model(images)
            loss, loss_rgb = criterion(output, labels)
